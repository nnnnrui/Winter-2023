...
class MuskDataModule(LightningDataModule):
    def __init__(
        self,
        data_dir: str = "data/musk",
        batch_size: int = 32,
        num_workers: int = 8,
        transform=None,
        **kwargs,
        ...
        super().__init__()
        self.dataset = MuskDataset(data_dir, transform=transform)
    ...
    def get_split(self):
        if osp.exists(f"{self.data_dir}/processed/split.pt"):
            ...
            skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=432)
            split = skf.split(self.dataset)
            torch.save(split, f"{self.data_dir}/processed/split.pt")
        return split

    def setup(self, stage: Optional[str] = None):
        dataset = MuskDataset(self.data_dir, transform=self.transform)
